...
do{\
    lv_obj_t* spinbox = ui->ctrl_pad.tab.layout.spinbox_##item;\
    lv_coord_t value = lv_obj_get_style_##item(obj, LV_PART_MAIN);\
...
static void ctrl_pad_spinbox_event_handler(lv_event_t* e)
{
    ...
    lv_style_prop_t* prop_p = lv_obj_get_user_data(spinbox);
    ...
    if (ui->obj_cur)
    {
        lv_obj_set_local_style_prop(ui->obj_cur, *prop_p, v, LV_PART_MAIN);
    }
}
