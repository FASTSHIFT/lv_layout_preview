...
do {\
    ui->ctrl_pad.tab.align.roller_align_##item = roller_create(\
        tab,\
        "Align "#item" place",\
        "FLEX_ALIGN_START\n"\
        "FLEX_ALIGN_END\n"\
...
static lv_obj_t* spinbox_ctrl_create(lv_obj_t* par, const char* txt, lv_style_prop_t prop)
{
    ...
    //lv_obj_remove_style_all(cont_main);
    ...
    lv_style_prop_t* prop_p = lv_mem_alloc(sizeof(lv_style_prop_t));
    if (prop_p)
    {
        *prop_p = prop;
    }
    ...
    lv_obj_set_user_data(spinbox, prop_p);
    lv_spinbox_set_range(spinbox, -4096, 4096);
...
void view_ctrl_pad_create(lv_obj_t* par, view_t* ui)
{
    ...
    lv_obj_set_size(tv, lv_pct(80), lv_pct(70));
    lv_obj_set_style_radius(tv, 10, 0);
    lv_obj_set_style_clip_corner(tv, true, 0);
